...
- The node process (`swe_af/app.py`) registers as `swe-planner`.
...
`execute` uses `swe_af/execution/dag_executor.py` and supports:
...
`swe_af/agent_ai/` provides provider-neutral orchestration with adapters for:
